...
const GroupCall = () => {
  ...
  const agendaCall = () => {
    try {
      fetch(URL + `/api/getCallInfo/${room.roomID}/${room.userID}`)
        .then(response => response.json())
        .then(data => {
          addAgenda(data?.Data?.call?.sessionName ?? meetInfo?.call?.interviewAgenda?.sessionName)
        })
        .catch((error) => {
  ...
  const addAgenda = (txt) => {
    ...
    //  agendaDiv.classList.add("sessionName")
  ...
  let myMeeting = async (element) => {
    ...
    zp.joinRoom({
      ...
      onLeaveRoom: () => {
        ...
        newButton.style.cssText = "margin-top: 20px; margin-bottom: 20px; cursor: pointer;width: 155px;height: 36px;font-weight: 500;font-size: 16px;line-height: 46px;border: 0;border-radius: 10px; color: #ffffff;background: #f00;display: flex;justify-content: center;align-items: center;";
        const newButtonMentee = document.createElement("button");
        newButtonMentee.innerHTML = "Give Feedback";
        newButtonMentee.style.cssText = "cursor:pointer;background-color:rgb(26, 115, 232);color:white;padding:15px 30px;margin-top:20px;font-weight:500;border:none;font-size:14px;border-radius:8px;";
        //creating a new button join
        const newButton2 = document.createElement("button");
        newButton2.innerHTML = "Rejoin";
        newButton2.style.cssText = "margin-top: 10px; cursor: pointer;width: 155px;height: 36px;font-weight: 500;font-size: 16px;line-height: 46px;border: 0;border-radius: 10px; color: #ffffff;background: #0e77d3;display: flex;justify-content: center;align-items: center;";
        ...
        if (isMentor) {
          ...
          homeButton.after(newButtonMentee);
        }
        newButton.addEventListener("click", () => {
          setShowPopup(true)
          mentorRedirectUrl()
        });
        newButtonMentee.addEventListener("click", () => {
          setShowPopup(true)
          redirectUrl()
        });
  ...
  useEffect(() => {
    ...
    fetch(URL + `/api/getCallInfo/${room.roomID}/${room.userID}`)
      ...
      .then(data => {
        setLoding(false)
        // Use the data from the API
        if (room.userID === data?.Data?.call?.mentor?.id) {
          setIsMentor(true)
          console.log("the user is mentor");
        }
        if (data?.Data?.call?.isGroupSession) {
          setIsValid(true)
        }
  ...
  useEffect(() => {
    ...
    if (currentTime > endTime) {
      ...
      if (meetInfo?.call?.sessionName || meetInfo?.call?.interviewAgenda?.sessionName) {
        console.log('The end time has not yet passed.', meetInfo?.call?.interviewAgenda?.sessionName);
        setIsEle(true);
        // addingAgenda();
        addAgenda(meetInfo?.call?.sessionName ?? meetInfo?.call?.interviewAgenda?.sessionName);
      }
    }
    ...
    console.log('Timepass.', meetInfo?.call?.meetingStatus);
  }, [meetInfo]);
  ...
  useEffect(() => {
    ...
    `;
      if (meetInfo?.call?.mentor?.id != room.userID) ele?.appendChild(newDiv3)
    ...
    "> 
     Agenda : <span style=" font-weight: 500;
        ...
        width: 100%;
        color: #737373;
        max-width: calc(100% - 64px);">${(meetInfo?.call?.sessionName) ?? (meetInfo?.call?.interviewAgenda?.sessionName)}</span>
    </div>
`;
    ...
    <>
      {(meetInfo && isValid && !timeEnd) ?
              ...
              <div className="popupWrapper logoutPop">
                <div className="popupContainer LogoutPopup" style={{ background: "#ffffffc9", borderColor: "#ffffffc9" }}>
                  ...
                  <div className="popupBody flexbox">
                    ...
                    <section className="dotscontainerLoad" >
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", width: "100%", minHeight: "100vh", flexDirection: "column", gap: "20px" }}>
          ...
          <h1 style={{ fontSize: "18px", fontWeight: "600", color: "#141414", textAlign: "center" }}>{loding ? <AdLoader /> : (!isValid && meetInfo ? "No group-call found with this meeting-id" : (timeEnd ? "The meeting is completed" : message))}</h1>
        </div>
